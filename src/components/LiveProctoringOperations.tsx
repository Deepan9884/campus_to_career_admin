...
export function LiveProctoringOperations({
  ...
  const blockExamCandidateMutation = useMutation({
    ...
    onSuccess: (res) => {
      toast.success(res.message || "Candidate disqualified and blocked!");
  ...
  const batchUnblockMutation = useMutation({
    mutationFn: (ids: string[]) => batchUnblockStudents(ids, masterUnblockReason || "Mentor proctoring audit reset"),
    onSuccess: (res) => {
      toast.success(res.message || "Exam access restored for selected candidates!");
  ...
  // Filter candidates in current exam room
  ...
  return (
    <div
      onClick={onClose}
      className="fixed inset-0 z-[99999] bg-black/85 backdrop-blur-md flex items-center justify-center p-3 sm:p-6 animate-in fade-in duration-150 select-none overflow-y-auto"
    >
      <div
        onClick={(e) => e.stopPropagation()}
        className="w-full max-w-6xl bg-slate-900 text-slate-100 border border-slate-700/80 rounded-3xl shadow-[0_0_80px_rgba(244,63,94,0.2)] overflow-hidden flex flex-col max-h-[94vh] my-auto backdrop-blur-2xl"
      >
        {/* ── TOP BAR / HEADER ────────────────────────────────────────── */}
          ...
          <div className="flex items-center gap-3">
            {selectedExamId ? (
              <button
                ...
                className="p-2.5 rounded-2xl bg-slate-800 hover:bg-slate-700 text-slate-300 hover:text-white transition flex items-center gap-1.5 font-bold text-xs cursor-pointer border border-slate-700"
              >
          ...
          <div className="flex items-center gap-2.5">
            {selectedExamId && (
              <button
                ...
                className="btn-gradient px-3.5 py-1.5 rounded-xl text-xs font-bold text-white flex items-center gap-1.5 shadow-md hover:opacity-95 transition cursor-pointer"
              >
                <FileText className="h-3.5 w-3.5" />
                <span>View Exam Report (PDF)</span>
              </button>
            )}
        ...
        {!selectedExamId ? (
          <div className="flex-1 overflow-y-auto p-6 space-y-6 text-xs select-text">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              <div className="p-4 rounded-2xl bg-slate-950/60 border border-slate-800 space-y-1">
                ...
                <p className="text-2xl font-black text-white">{examsWithTakers.length}</p>
                <p className="text-[10px] text-slate-500">Active & Scheduled modules</p>
              </div>
              ...
              <div className="p-4 rounded-2xl bg-slate-950/60 border border-slate-800 space-y-1">
                ...
                <p className="text-2xl font-black text-emerald-400">{totalActiveCandidates}</p>
                <p className="text-[10px] text-slate-500">Actively taking tests right now</p>
              </div>
              ...
              <div className="p-4 rounded-2xl bg-slate-950/60 border border-slate-800 space-y-1">
                ...
                <p className="text-[10px] text-slate-500">1-2 strikes recorded</p>
              </div>
              ...
              <div className="p-4 rounded-2xl bg-slate-950/60 border border-slate-800 space-y-1">
                ...
                <p className="text-2xl font-black text-rose-400">{blockedUsers.length}</p>
                <p className="text-[10px] text-slate-500">3 strikes / full-screen timeout</p>
              </div>
            </div>
            ...
            <div className="space-y-4">
              ...
              {examsWithTakers.length === 0 ? (
                <div className="p-12 text-center bg-slate-950/40 rounded-3xl border border-slate-800 text-slate-400 space-y-3">
                  <CheckCircle2 className="h-8 w-8 text-emerald-400 mx-auto" />
                  <div>
                    <h4 className="text-sm font-bold text-white">No Active Exam Sessions</h4>
                  ...
                  <button
                    ...
                    className="btn-gradient px-4 py-2 rounded-xl text-xs font-bold text-white shadow"
                  >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {examsWithTakers.map((exam) => (
                    <div
                      ...
                      className="p-5 rounded-3xl bg-slate-950/70 border border-slate-800/90 hover:border-indigo-500/60 shadow-lg hover:shadow-indigo-500/10 transition-all duration-200 cursor-pointer group flex flex-col justify-between space-y-4"
                    >
                      <div className="space-y-2.5">
                        <div className="flex items-start justify-between gap-3">
                          <div className="flex items-center gap-2.5">
                            <div className="p-2.5 rounded-2xl btn-gradient text-white shadow-md">
                              <FileCode className="h-4 w-4" />
                            </div>
                            <div>
                              <h4 className="font-black text-sm text-white group-hover:text-indigo-400 transition">
                                {exam.examTitle}
                              </h4>
                        ...
                        <div className="grid grid-cols-3 gap-2 pt-1">
                          <div className="p-2.5 rounded-xl bg-slate-900 border border-slate-800 text-center">
                            <p className="text-lg font-black text-emerald-400">{exam.activeCount}</p>
                            <p className="text-[9px] uppercase font-bold text-slate-400">Active Online</p>
                          </div>
                          <div className="p-2.5 rounded-xl bg-slate-900 border border-slate-800 text-center">
                            <p className="text-lg font-black text-amber-400">{exam.warningCount || 0}</p>
                            <p className="text-[9px] uppercase font-bold text-slate-400">Warnings</p>
                          </div>
                          <div className="p-2.5 rounded-xl bg-slate-900 border border-slate-800 text-center">
                            <p className="text-lg font-black text-rose-400">{exam.blockedCount || 0}</p>
                            <p className="text-[9px] uppercase font-bold text-slate-400">Blocked</p>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center justify-between pt-2 border-t border-slate-800/80 text-xs">
                        <span className="text-[11px] text-slate-400 flex items-center gap-1">
                          ...
                          <span>Status: <strong className="text-white capitalize">{exam.status}</strong></span>
                        </span>
            ...
            {blockedUsers.length > 0 && (
              <div className="p-4 rounded-3xl bg-slate-950/60 border border-rose-500/20 space-y-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                ...
                <div className="divide-y divide-slate-800 rounded-2xl border border-slate-800 overflow-hidden bg-slate-900/80">
                  {blockedUsers.map((u) => (
                    <div key={u._id} className="p-3 flex items-center justify-between hover:bg-slate-800/50 transition">
                      <div className="flex items-center gap-2.5">
                        <input
                          type="checkbox"
                        ...
                        <div>
                          ...
                          <p className="text-[10px] text-slate-400">{u.email} • {u.targetRole}</p>
                        </div>
                      </div>
                ...
                {selectedMasterBlockedIds.length > 0 && (
                  <div className="flex items-center justify-between gap-3 pt-2">
                    ...
                    <button
                      ...
                      disabled={batchUnblockMutation.isPending}
                      className="btn-gradient px-4 py-1.5 rounded-xl text-xs font-bold text-white flex items-center gap-1.5 shadow"
                    >
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-5 text-xs select-text">
            ...
            <div className="p-4 rounded-2xl bg-slate-950/80 border border-slate-800 flex flex-col md:flex-row items-center justify-between gap-3">
              {/* Search */}
              <div className="relative w-full md:w-72">
                ...
                <input
                  ...
                  className="w-full pl-9 pr-3 py-2 rounded-xl bg-slate-900 border border-slate-700 text-xs text-white placeholder:text-slate-500 focus:outline-none focus:border-indigo-500"
                />
              </div>
              ...
              <div className="flex items-center gap-1.5 overflow-x-auto w-full md:w-auto pb-1 md:pb-0">
                <button
                  ...
                  className={`px-3 py-1.5 rounded-xl font-bold transition cursor-pointer shrink-0 ${
                    statusFilter === "all" ? "btn-gradient text-white shadow" : "bg-slate-900 text-slate-400 border border-slate-800"
                  }`}
                >
                ...
                <button
                  ...
                  className={`px-3 py-1.5 rounded-xl font-bold transition cursor-pointer shrink-0 ${
                    statusFilter === "active" ? "btn-gradient text-white shadow" : "bg-slate-900 text-slate-400 border border-slate-800"
                  }`}
                >
                ...
                <button
                  ...
                  className={`px-3 py-1.5 rounded-xl font-bold transition cursor-pointer shrink-0 ${
                    statusFilter === "warning" ? "btn-gradient text-white shadow" : "bg-slate-900 text-slate-400 border border-slate-800"
                  }`}
                >
                ...
                <button
                  ...
                  className={`px-3 py-1.5 rounded-xl font-bold transition cursor-pointer shrink-0 ${
                    statusFilter === "blocked" ? "btn-gradient text-white shadow" : "bg-slate-900 text-slate-400 border border-slate-800"
                  }`}
                >
              ...
              <select
                ...
                className="px-3 py-2 rounded-xl bg-slate-900 border border-slate-700 text-xs text-white outline-none focus:border-indigo-500 cursor-pointer"
              >
            ...
            {currentCandidates.length === 0 ? (
              <div className="p-12 text-center bg-slate-950/40 rounded-3xl border border-slate-800 text-slate-400 space-y-2">
                <CheckCircle2 className="h-7 w-7 text-emerald-400 mx-auto" />
                <h4 className="font-bold text-white">No candidates match this filter</h4>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {currentCandidates.map((cand) => (
                  <div
                    ...
                    className={`p-4 rounded-3xl border transition-all duration-200 space-y-3.5 shadow-md ${
                      cand.status === "blocked"
                  ...
                  >
                    ...
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex items-center gap-3">
                        {cand.avatar ? (
                          <img src={cand.avatar} alt={cand.name} className="h-10 w-10 rounded-2xl object-cover border border-slate-700" />
                        ) : (
                          <div className="h-10 w-10 rounded-2xl btn-gradient flex items-center justify-center text-white font-bold text-sm">
                            {cand.name.charAt(0)}
                          </div>
                        )}
                        <div>
                    ...
                    <div className="space-y-1.5">
                      <div className="flex items-center justify-between text-xs">
                        <span className="text-slate-400 text-[11px] font-bold">Proctoring Trust Score</span>
                        <span
                    ...
                    <div className="grid grid-cols-3 gap-2 text-[10px]">
                      ...
                      <div className="p-2 rounded-xl bg-slate-900/90 border border-slate-800 text-center">
                        <span className="text-slate-500 block">Eye Gaze Alerts</span>
                        <strong className="text-slate-200 font-mono text-xs">
                          {cand.violationDetails?.filter((v: any) => v.violationType === "eye_tracking_violation")?.length || 0}
                        </strong>
                      </div>
                      <div className="p-2 rounded-xl bg-slate-900/90 border border-slate-800 text-center">
                        <span className="text-slate-500 block">Face Alerts</span>
                        <strong className="text-slate-200 font-mono text-xs">
                      ...
                      <div className="flex items-center gap-2">
                        {cand.status === "blocked" ? (
                          ...
                          <button
                            ...
                            className="px-3 py-1.5 rounded-xl bg-rose-600/20 hover:bg-rose-600 text-rose-300 hover:text-white border border-rose-500/30 font-bold text-xs flex items-center gap-1 transition cursor-pointer"
                          >
        ...
        {selectedCandidateForTimeline && (
          ...
          >
            <div
              onClick={(e) => e.stopPropagation()}
              className="w-full max-w-lg bg-slate-900 border border-slate-700 rounded-3xl p-6 space-y-4 shadow-2xl"
            >
              <div className="flex items-center justify-between border-b border-slate-800 pb-3">
                <div className="flex items-center gap-2.5">
                  <Activity className="h-5 w-5 text-indigo-400" />
                  <div>
                    <h3 className="font-bold text-sm text-white">{selectedCandidateForTimeline.name}</h3>
                    <p className="text-[11px] text-slate-400">Live Examination Telemetry & Violation Log</p>
                  </div>
                </div>
                <button
                  onClick={() => setSelectedCandidateForTimeline(null)}
              ...
              <div className="space-y-2 max-h-72 overflow-y-auto pr-1">
                {(!selectedCandidateForTimeline.violationDetails ||
                  ...
                  <div className="p-6 text-center text-slate-400 space-y-1">
                    ...
                    <p className="font-bold text-slate-300">Clean Examination Session</p>
                    <p className="text-[11px] text-slate-500">Zero suspicious activity or violations recorded so far.</p>
                  </div>
                ) : (
        ...
        {blockingCandidate && (
          ...
          >
            <div
              onClick={(e) => e.stopPropagation()}
              className="w-full max-w-md bg-slate-900 border border-rose-500/40 rounded-3xl p-6 space-y-4 shadow-2xl"
            >
              <div className="flex items-center gap-3 text-rose-400">
                ...
                <div>
                  <h3 className="font-black text-sm text-white">Disqualify & Block Examinee</h3>
                  <p className="text-xs text-rose-300/80">Immediate examination revocation</p>
                </div>
              </div>
              ...
              <div>
                <label className="text-[11px] font-bold text-slate-400 block mb-1">Reason for Disqualification:</label>
                <input
                  type="text"
                  value={blockReasonInput}
                  onChange={(e) => setBlockReasonInput(e.target.value)}
                  placeholder="e.g. Unauthorized materials detected, Tab switching"
                  className="w-full px-3 py-2 rounded-xl bg-slate-950 border border-slate-700 text-xs text-white placeholder:text-slate-500 outline-none focus:border-rose-500"
                />
              </div>
              ...
              <div className="flex items-center justify-end gap-2 pt-2 border-t border-slate-800">
                ...
                <button
                  ...
                  className="px-4 py-2 rounded-xl bg-rose-600 hover:bg-rose-500 text-white text-xs font-bold flex items-center gap-1.5 shadow cursor-pointer"
                >
