...
export function MentorProductTour({ open, onClose }: MentorProductTourProps) {
  ...
  // Target element locator with smooth scroll
  const updateTargetLocation = useCallback(() => {
    ...
    if (el) {
      ...
      const isComfortablyVisible =
        elRect.top >= 80 &&
        elRect.bottom <= window.innerHeight - 80 &&
        elRect.left >= 20 &&
  ...
  }, [open, currentStep, location.pathname, isNavigating, updateTargetLocation]);
  ...
  return createPortal(
    <div className="fixed inset-0 z-[99999] overflow-hidden font-sans select-none">
      ...
      {/* Pulsing Glowing Beacon Ring around Target Element */}
      {targetRect && !isNavigating && (
        <div
          style={{
          ...
          className="fixed z-40 pointer-events-none rounded-2xl border-2 border-indigo-500 ring-4 ring-indigo-500/30 shadow-[0_0_40px_rgba(99,102,241,0.5)] transition-all duration-500 ease-in-out animate-pulse"
        />
      )}
